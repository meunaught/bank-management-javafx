...
public class CurrentAccount extends Account{
    ...
    public CurrentAccount(String type,long number,long time,double balance,double main_balance,double withdraw_amount)
    {
        setType(type);
        setNumber(number);
        setTime(time);
    ...
    public void createBalance()
    {
        ...
        if(hours>=1)
        {
            account.setBalance(account.getBalance()+ (account.getBalance()*Math.floor(hours)*account.getInterest_rate())/100.0);
