...
public class DashboardController extends Controller implements Initializable{
    ...
    @FXML
    private JFXTreeTableColumn<myCustomer, String> phoneColumn;
    @FXML
    private JFXTreeTableColumn<myCustomer, String> statusColumn;
    ...
    static final class myCustomer extends RecursiveTreeObject<myCustomer> {
        ...
        myCustomer(Customer customer) {
            ...
            this.list[1] = new SimpleStringProperty((customer.getAccount().getType()));
            this.list[2] = new SimpleStringProperty(Long.toString(customer.getAccount().getNumber()));
            this.list[3] = new SimpleStringProperty(Double.toString(customer.getAccount().getBalance()));
            this.list[4] = new SimpleStringProperty(Long.toString(customer.getPhone()));
            this.list[5] = new SimpleStringProperty(customer.getAccount().getStatus());
        }
    ...
    private ObservableList<myCustomer> genCell() {
        ...
        var ccList = dbcontroller.getInstance().customerArrayList();
        for(Customer cc : ccList) {
            cc.getAccount().createBalance();
//            System.out.println(cc.getAccount().getBalance());
    ...
    private void setupReadOnlyTableView() {
        ...
        setupCellValueFactory(balanceColumn, myCustomer::balance);
        setupCellValueFactory(phoneColumn, myCustomer::phone);
        setupCellValueFactory(statusColumn, myCustomer::status);
